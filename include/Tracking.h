...
#include "PointCloudMapper.h"  //添加ht
...
public: //添加 ht
    ...
    cv::Mat LKimg;
private: //添加 ht
    std::mutex mtxLK;  // 声明一个全局的互斥锁
    std::mutex mMutexPose;  // 用于位姿计算的锁
    std::mutex mMutexFrame; // 用于帧数据保护的锁
...
public:
    ...
    cv::Mat mImGray;
    cv::Mat mImDepth;  //添加 ht

    PointCloudMapper* mpPointCloudMapper; //添加ht
    cv::Mat mImColor;  //添加ht
    cv::Mat mDepth; //添加ht
...
protected:
    ...
    bool mbUseMotionCompensation;  // v4.0新增：是否启用相机运动补偿
    ...
    int last_mnMatchesInliers;  //添加 ht
