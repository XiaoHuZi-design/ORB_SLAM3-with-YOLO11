...
*
* ORB-SLAM3 is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; but WITHOUT EVEN
    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    cout<<"YoloDetector initialized!"<<endl;
    ...
    mpPointCloudMapper = new PointCloudMapper();   //添加ht
...
//加入了双目模式中对于颜色通道的判断
    ...
    else if (imLeft.channels() == 1)
        {imLeftGray = imLeft.clone();}
    else
    {
        cerr << "ERROR: the input images are not grayscale nor RGB." << endl;
        exit(-1);
    }
    if(imRight.channels() == 3)
        {cv::cvtColor(imRight, imRightGray, cv::COLOR_BGR2GRAY);}
    else if (imRight.channels() == 1)
    ...
    if(settings_ && settings_->needToRectify()){
        ...
        cv::remap(imLeftGray, imLeftToFeed, M1l, M2l, cv::INTER_LINEAR);
        cv::remap(imRightGray, imRightToFeed, M1r, M2r, cv::INTER_LINEAR);
    }
    else if(settings_ && settings_->needToResize()){
        cv::resize(imLeftGray,imLeftToFeed,settings_->newImSize());
        cv::resize(imRightGray,imRightToFeed,settings_->newImSize());
    }
    else{
        imLeftToFeed = imLeftGray.clone();
        imRightToFeed = imRightGray.clone();
    }
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageStereo(imLeftToFeed,imRightToFeed,timestamp,filename,imLeft);
