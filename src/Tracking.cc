    ...
    mstrSettingsPath(strSettingPath)  // 保存配置文件路径
{
    ...
    if(settings){
        ...
        // Load DynamicFilter parameters (新增)
        ...
        if (!fnDynamicFilter.empty() && (int)fnDynamicFilter == 1) {
            ...
            std::cout << "[Tracking] DynamicFilter.UseFilterManager enabled" << std::endl;
        } else {
            mbUseFilterManager = false;
            std::cout << "[Tracking] DynamicFilter.UseFilterManager disabled (using pure YOLO)" << std::endl;
        }
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename,const cv::Mat &imLeft)
{
    ...
    cv::Mat InputImage;
    //InputImage = imLeft.clone();
    //mpDetector->GetImage(InputImage);
//*********************************************************************************************************************** */
//加入了颜色通道的处理
    if (imLeft.channels() == 1) {
         // 确保转换为3通道彩色图
         cv::cvtColor(imLeft, InputImage, cv::COLOR_GRAY2BGR);
            std::cout << "WARNING: Converted grayscale image to BGR for YOLO detection" << std::endl;
} 
        else if (imLeft.channels() == 3) {
            InputImage = imLeft.clone();
}
        else if (imLeft.channels() == 4) {
         // 处理带透明通道的情况
            cv::cvtColor(imLeft, InputImage, mbRGB ? cv::COLOR_RGBA2BGR : cv::COLOR_BGRA2BGR);
}
    else {
    std::cerr << "ERROR: Unexpected image channels (" << imLeft.channels() << ")" << std::endl;
    throw std::runtime_error("Invalid image format for YOLO detector");
}
...
//         // mCurrentFrame.mTcw = mTcw;  // mTcw 设置的protect私有成员变量，访问不到
//         //mCurrentFrame.GetPose() = mTcw;   //orbslam2中mTcw为opencv的mat格式,orbslam3中mTcw为Sophus::SE3<float>  需要转化
...
//         needNewKF = NeedNewKeyFrame();
// */  
//         mCurrentFrame = Frame(true);  //添加Frame::Frame(bool idPlus)
//         cv::Mat cvTcw;  // 使用临时变量存储OpenCV格式的位姿
//         last_mnMatchesInliers = mnMatchesInliers;

//         // 计算光流并获取位姿
//         LKimg = computeMtcwUseLK(mpLastKeyFrame, imRGB,  
//             mCurrentFrame.mnId - mpLastKeyFrame->mnFrameId ==1, 
//             mK, mDistCoef, cvTcw, mnMatchesInliers);

//         mpFrameDrawer->LK = LKimg;

//         // 转换为Sophus格式并设置位姿
//         Sophus::SE3<float> se3Tcw = CvMatToSE3(cvTcw);
//         mCurrentFrame.SetPose(se3Tcw);  // 使用SetPose方法而非直接赋值

//         // 计时输出
//         clock_t b = clock();
//         cout<<"track LK optical flow use time:"
//             <<1000*(float)(b-a)/CLOCKS_PER_SEC<<"ms"<<endl;
...
{
    mImGray = imRGB;
    mImColor = imRGB.clone();  //添加ht map

    cv::Mat imDepth = imD;
    //mImDepth = imD; //添加
   
    //检查图像是否为空
    if(mImGray.empty() || imDepth.empty()) {
        cout << "ERROR1: Empty input images for keyframe creation!" << endl;
    }

    // ========== 动态物体检测：YOLO + 光流 ==========
    // 重要：避免重复检测！
    // - 使用FilterManager时：YOLO在YoloFilter内部调用（YoloFilter.cpp:94）
    // - 不使用FilterManager时：直接调用YOLO（见else分支）
    ...
    if (mpFilterManager && mbUseFilterManager) {
        ...
        if (mbUseMotionCompensation && !mLastFrame.GetPose().matrix().isZero(0)) {
            // 计算相机运动：T_curr * T_prev^-1
            Sophus::SE3f T_prev = mLastFrame.GetPose();
            Sophus::SE3f T_curr = mCurrentFrame.GetPose();  // 注意：此时mCurrentFrame还未创建，先使用估计值

            // 第一帧或位姿无效时，回退到不带补偿的检测
            if (T_prev.matrix().isZero(0) || mState == NO_IMAGES_YET || mState == NOT_INITIALIZED) {
                std::cout << "[Tracking] v4.0: First frame or invalid pose, using FilterFrame without motion compensation" << std::endl;
                ...
                if (!mVelocity.matrix().isZero(0)) {
                    // 使用速度模型：T_curr_estimated = T_velocity * T_prev
                    camera_motion = mVelocity;
                } else {
                    // 没有速度模型，假设静止（identity transformation）
                    camera_motion = Sophus::SE3f();
                }

                std::cout << "[Tracking] v4.0: Using motion compensation, camera motion magnitude: "
            ...
            // 不使用相机运动补偿 
            dynamicAreas = mpFilterManager->FilterFrame(InputImage, timestamp);//YOLO+光流
        }
        std::cout << "[Tracking] FilterManager detected " << dynamicAreas.size() << " dynamic areas" << std::endl;
    } else {
        // 回退到纯YOLO检测（原有逻辑）
        // 注意：此分支只在不使用FilterManager时执行
        mpDetector->GetImage(InputImage);
        mpDetector->Detect();
    ...
    {
        std::unique_lock<std::mutex> lock(mpViewer->mMutexPAFinsh);
        if (!mbUseFilterManager) {
            // ========== 原有逻辑：纯YOLO检测 ==========
            mpViewer->mmDetectMap = mpDetector->mmDetectMap;
            mpViewer->mvYoloAreas.clear();
            mpViewer->mvOpticalAreas.clear();
            mpViewer->mvMergedAreas.clear();
        } else {
            // ========== 新增：FilterManager检测，分别存储各过滤器结果 ==========
            // 1. 存储YOLO的mmDetectMap用于显示类别名称
            mpViewer->mmDetectMap = mpDetector->mmDetectMap;

            // 2. 获取各过滤器的独立检测结果
            auto individual_results = mpFilterManager->GetIndividualResults();

            // 3. 分别存储YOLO、光流、融合结果
            if (individual_results.find("yolo") != individual_results.end()) {
                mpViewer->mvYoloAreas = individual_results["yolo"];
            } else {
                mpViewer->mvYoloAreas.clear();
            }

            if (individual_results.find("optical_flow") != individual_results.end()) {
                mpViewer->mvOpticalAreas = individual_results["optical_flow"];
            } else {
                mpViewer->mvOpticalAreas.clear();
            }

            mpViewer->mvMergedAreas = dynamicAreas;  // 融合后的结果

            std::cout << "[Tracking] Visualization: YOLO=" << mpViewer->mvYoloAreas.size()
                      << " OpticalFlow=" << mpViewer->mvOpticalAreas.size()
                      << " Merged=" << mpViewer->mvMergedAreas.size() << std::endl;
        }
    }
    // 清空检测结果  注释
    // mpDetector->mvDynamicArea.clear();
    ...
    // 关键修改：在清空之前将动态区域赋值给当前帧 注意顺序 不然结果是没有剔除的点云地图
    mCurrentFrame.mvDynamicArea = dynamicAreas; //添加ht map

    // 清空检测结果（只在使用原YOLO时）
    if (!mbUseFilterManager) {
        mpDetector->mmDetectMap.clear(); //添加ht map
        mpDetector->mvDynamicArea.clear(); //添加ht map
    }
    ...
    /*********************************LK光流 - 暂时禁用，需要重新设计集成方式*****************************************/
    // 问题：LK光流创建新Frame会干扰正常的Track()流程
    // TODO: 需要重新设计LK光流的集成方式
    /*
    if(mState == OK && mpLastKeyFrame)
    {
        std::unique_lock<std::mutex> lock(mtxLK);
        needNewKF = NeedNewKeyFrame();
        ...
        if(!needNewKF)
        {
            ...
            LKimg = computeMtcwUseLK(mpLastKeyFrame, mImColor,
                mCurrentFrame.mnId - mpLastKeyFrame->mnFrameId == 1,
                mK, mDistCoef, cvTcw, mnMatchesInliers);
...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    mpDetector->mvDynamicArea.clear();
    mpDetector->mmDetectMap.clear();
//检测输入图像的色彩通道，如果是3通道，转换为灰度图像，否则不转换

    if(im.channels()!=1)//检测是否输入的是灰度？
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    // 检查当前传感器是否为IMU相关类型(单目+IMU/双目+IMU/RGBD+IMU)且IMU尚未初始化完成
    if((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD) && !mpAtlas->GetCurrentMap()->isImuInitialized())
    {
        // 如果是IMU单目相机且距离上一关键帧超过0.25秒，需要创建新关键帧
        if (mSensor == System::IMU_MONOCULAR && (mCurrentFrame.mTimeStamp-mpLastKeyFrame->mTimeStamp)>=0.25)
            return true;
        // 如果是IMU双目或IMU RGBD相机且距离上一关键帧超过0.25秒，需要创建新关键帧  
    ...
    // 步骤5：统计当前帧中已被跟踪的近距离点的数量，以及可能新增的近距离点的数量。
    int nNonTrackedClose = 0; // 未被跟踪的近距离点计数
    int nTrackedClose= 0; // 已被跟踪的近距离点计数
    // 仅对非单目相机(双目/RGBD等)进行统计
    if(mSensor!=System::MONOCULAR && mSensor!=System::IMU_MONOCULAR)
    {
        // 确定要处理的特征点数量(考虑左右目)
        ...
        for(int i =0; i<N; i++)
        {
            // 检查是否为有效近距离点(深度在0~mThDepth范围内)
            if(mCurrentFrame.mvDepth[i]>0 && mCurrentFrame.mvDepth[i]<mThDepth)
            {
                // 如果是有效地图点且非异常点
                if(mCurrentFrame.mvpMapPoints[i] && !mCurrentFrame.mvbOutlier[i])
                    nTrackedClose++; // 增加已跟踪计数
                else
                    nNonTrackedClose++; // 增加未跟踪计数
    ...
    bool bNeedToInsertClose;  // 判断是否需要插入近距离关键帧的条件：
    bNeedToInsertClose = (nTrackedClose<100) && (nNonTrackedClose>70); // 已跟踪近距离点<100 且 未跟踪近距离点>70
    ...
    // 跟踪要跪的节奏，0.25是一个比较低的阈值  传感器类型条件：非单目/非IMU单目/非IMU双目/非IMU-RGBD  匹配点条件：内点匹配数不足参考值的25% 或 需要插入近距离关键帧
    const bool c1c = mSensor!=System::MONOCULAR && mSensor!=System::IMU_MONOCULAR && mSensor!=System::IMU_STEREO && mSensor!=System::IMU_RGBD && (mnMatchesInliers<nRefMatches*0.25 || bNeedToInsertClose) ;  //注释
    // Condition 2: Few tracked points compared to reference keyframe. Lots of visual odometry compared to map matches.
    // 阈值比c1c要高，与之前参考帧（最近的一个关键帧）重复度不是太高
    const bool c2 = (((mnMatchesInliers<nRefMatches*thRefRatio || bNeedToInsertClose)) && mnMatchesInliers>15); //注释
    ...
    const bool c5 = last_mnMatchesInliers/mnMatchesInliers>2;///inlier decrease fast  ht 添加  

    if(((c1a||c1b||c1c) && c2)||c3 ||c4)
    // if(((c1a||c1b||c1c) && c2)||c3 ||c4 ||c5)
    // if(( (c1a||c1b)&&c3 )||(c4||c5))  //LK 改 
    {
...
void Tracking::CreateNewKeyFrame()
{
    ...
    pKF->mvDynamicArea = mCurrentFrame.mvDynamicArea; //添加ht map
    // 添加调试输出
    cout << "=== KeyFrame Creation Debug ===" << endl;
    cout << "KeyFrame ID: " << pKF->mnId << endl;
    cout << "CurrentFrame Dynamic Areas: " << mCurrentFrame.mvDynamicArea.size() << endl;
    cout << "KeyFrame Dynamic Areas: " << pKF->mvDynamicArea.size() << endl;

    // 打印前几个区域的信息
    for (size_t i = 0; i < min((size_t)3, pKF->mvDynamicArea.size()); ++i) {
        const cv::Rect& area = pKF->mvDynamicArea[i];
        cout << "Area " << i << ": [" << area.x << ", " << area.y << ", " 
            << area.width << ", " << area.height << "]" << endl;
    }

    mpLocalMapper->InsertKeyFrame(pKF);

    mpPointCloudMapper->InsertKeyFrame(pKF, this->mImColor, this->mDepth);  //添加ht map
